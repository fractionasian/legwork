...
test("graph cache HIT returns stored body, never calls Overpass", async () => {
  ...
  const env = mockEnv({ "g2:-31.950:115.860:2000": '{"elements":[1]}' });
  let called = false;
...
test("graph cache MISS fetches Overpass, returns it, and writes R2", async () => {
  ...
  assert.equal(env.store.get("g2:-31.950:115.860:2000"), '{"elements":[42]}', "R2 should be written under the snapped key");
});
...
test("graph returns 502 when the Overpass fetch THROWS (network unreachable)", async () => {
  ...
  assert.equal(env.store.has("g2:-31.950:115.861:2000"), false);
});
